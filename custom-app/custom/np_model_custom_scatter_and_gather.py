        ...
        try:
            ...
            while self._current_round < self._start_round + self._num_rounds:
                ...
                if self._current_round < self._start_round + self._num_rounds:
                    ...
                    self.log_info(fl_ctx, "Final validation Round is  started.")
                    fl_ctx.set_prop(AppConstants.GLOBAL_MODEL, self._global_weights, private=True, sticky=True)
                    fl_ctx.set_prop(AppConstants.CURRENT_ROUND, self._current_round, private=True, sticky=False)
                    self.fire_event(AppEventType.ROUND_STARTED, fl_ctx)
                    ...
                    validation_task = Task(
                        name=self.validation_task_name,
                        data=data_shareable,
                        props={},
                        timeout=self._train_timeout,
                        before_task_sent_cb=self._prepare_validation_task_data,
                        result_received_cb=self._process_validation_result,
                    )
                    ...
                    self.log_info(fl_ctx, "inal validation Round is  finished.")
...
#########################################################
    ...
    def _prepare_validation_task_data(self, client_task: ClientTask, fl_ctx: FLContext) -> None:
         fl_ctx.set_prop(AppConstants.TRAIN_SHAREABLE, client_task.task.data, private=True, sticky=False)
         self.fire_event(AppEventType.BEFORE_VALIDATE_MODEL, fl_ctx)

    def _process_validation_result(self, client_task: ClientTask, fl_ctx: FLContext) -> None:
         result = client_task.result
         client_name = client_task.client.name
    
         self._accept_validation_result(client_name=client_name, result=result, fl_ctx=fl_ctx)
    
         # Cleanup task result
         client_task.result = None
    
    def _accept_validation_result(self, client_name: str, result: Shareable, fl_ctx: FLContext) -> bool:
        ...
        self.log_info(fl_ctx, f"Contribution from {client_name} {accepted_msg} of validation task.")
