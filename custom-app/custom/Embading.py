...
class Graph_embading:
    ...
    def embed_graph(self,random_walks,com_round):
        ...
        if(com_round <5):
            ...
            epoch = 3
        elif(15<=com_round <22):
            epoch = 10
        elif(com_round >=22):
            epoch = 20
        model_w2v = Word2Vec(vector_size=100,window=4, sg = 1, hs = 0,
                         negative = 10, # for negative sampling
                         alpha=0.03, min_alpha=0.0007,
                         seed = 14)
        model_w2v.build_vocab(random_walks, progress_per=2)
        model_w2v.train(random_walks, total_examples = model_w2v.corpus_count, epochs=epoch, report_delay=1)
        return model_w2v
